...
impl R {
    #[doc = "Bits 0:31 - Disable APPROTECT and enable debug access to non-secure mode until next pin reset if KEY fields match The current APPROTECT value as configured from UICR is bypassed if value of KEY fields are non-zero and KEY fields match on both CPU and debugger side"]
    #[inline(always)]
